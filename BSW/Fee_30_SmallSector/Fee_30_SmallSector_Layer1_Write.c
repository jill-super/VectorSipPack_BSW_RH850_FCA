...
FEE_30_SMALLSECTOR_LOCAL FUNC (void, FEE_30_SMALLSECTOR_PRIVATE_CODE) Fee_30_SmallSector_Wr_ProcessInstanceFinderState(void)
{
  ...
  /* #10 Next step depends on InstanceFinder's result: */
  /* #11 - Erase dataset if InstanceFinder returns with MEMIF_BLOCK_INCONSISTENT which indicates that the dataset is invalid */
  /* #12 - Erase dataset if found instance is last instance in dataset */
  /* #13 - Erase dataset if found instance is first instance in dataset and if it's blank */
  /* #14 - Write instance otherwise */

  /* #20 Handle successful InstanceFinder jobs */
  switch (InstanceFinderResult)
  {
  case INSTANCE_FINDER_OK:
  {
    boolean IsLastInstance = Fee_30_SmallSector_Dh_IsLastInstance(&Fee_30_SmallSector_Wr_Instance) == TRUE; /* SBSW_FEE_30_SMALLSECTOR_81 */

    /* #30 Check if found instance is last instance in dataset or if found instance is first instance and blank*/
    if ((Fee_30_SmallSector_Dh_IsFirstInstanceAndBlank(&Fee_30_SmallSector_Wr_Instance) == TRUE) || (IsLastInstance == TRUE)) /* SBSW_FEE_30_SMALLSECTOR_81 */
    {
      /* #31 Initiate DatasetEraser job, because there's no more space for another instance in the dataset */
      Fee_30_SmallSector_Wr_InitiateDatasetErase(InstanceFinderResult);
    }
    else
    {
      /* #33 Unconditionally calculate next free position for the instance. At this point Fee_30_SmallSector_Wr_Instance is always NOT blank */
      /* #34 Theoretically, the only blank instance the InstanceFinder can return could be the first instance of a dataset, but in this case */
      /* #35 the dataset is erased completely and not handled here, according to positive if-clause */
      Fee_30_SmallSector_Dh_PrepareNextInstance(&Fee_30_SmallSector_Wr_Instance); /* SBSW_FEE_30_SMALLSECTOR_81 */

      /* #38 Initiate WriteInstance job */
      Fee_30_SmallSector_Wr_InitiateWriteInstance();
    }
    break;
  }
  ...
  case INSTANCE_FINDER_EP2_INCORRECT:
    /* #40 Initiate DatasetEraser job */
    Fee_30_SmallSector_Wr_InitiateDatasetErase(InstanceFinderResult);
    break;

  default: /* #50 Finish job due to failed InstanceFinder job: MEMIF_JOB_FAILED */
