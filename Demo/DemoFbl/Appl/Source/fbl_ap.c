...
#if defined( FBL_BM_ENABLE_BOOTMANAGER )
# define FBL_BMPRESPAT_START_SEC_CONST
# include "MemMap.h"
V_MEMROM0 V_MEMROM1 vuint8 V_MEMROM2 bootPresencePattern[kFblPresencePatternSize] = kFblPresencePattern;
# define FBL_BMPRESPAT_STOP_SEC_CONST
# include "MemMap.h"

# define FBL_BMINFO_START_SEC_CONST
# include "MemMap.h"
V_MEMROM0 V_MEMROM1 tBmInfoHeader V_MEMROM2 BootInfoHeader =
{
   /* Magic flag */
    0x12345678u

   /* Specify startup code address */
   ,FBL_STARTUP_ADDR

   /* Type of application: FBL */
   ,BmTargetFbl

   /* Type of validation: Always Presence patterns for FBL */
   ,BmValidPrescencePattern

   /* Presende Pattern address */
   ,(tBmAddress)bootPresencePattern

# if defined( FBL_BM_ENABLE_VERIFICATION )
   /* Verification information has to be updated in a post-build process */
   /* checkStartAddress */
   ,0x2038u
   
   /* checkLength, set length to 0 to skip verification in boot manager */
   ,0x00u
...
# define FBL_BMINFO_STOP_SEC_CONST
# include "MemMap.h"
#endif   /* FBL_BM_ENABLE_BOOTMANAGER */
...
void ApplFblInit( void )
{
	...
	/* Necessary precondition for MCAN: CLK_HSB > CLKP_H2 ! */
	  FBL_CKSC0C = 0x01ul;       /* Configure PLL as source clock for system and peripheral clock */
    while(FBL_CKSC0S != 0x01ul); /* PLL clock is selected */
	
	/* Configuration of System Clock Divider */
	/* Use 0x01ul for 240 MHz CLK_CPU, 80 Mhz CLK_HSB */ 
   FBL_CLKD0DIV = 0x04ul; /* CLK_CPU, CLK_HSB (STM Timer), CLK_LSB */
   while(FBL_CLKD0STAT!=0x03ul);  /* Clock divider 0 is running at the division ratio specified in CLKD0DIV (SYNC and ACTIVE) */
		
	/* Configuration of Peripheral Clock Divider */
   FBL_CLKD1DIV = 0x06ul; /* CLKP_C, CLKP_H1, CLKP_H2 (MCAN), CLKP_L (LIN) */
   while(FBL_CLKD1STAT != 0x03ul);  /* Clock divider 0 is running at the division ratio specified in CLKD1DIV (SYNC and ACTIVE) */
   /* On P1X-C derivatives, the clock to the peripheral units is cut after reset and has to be activated manually */
   /* CAN */
   FBL_MSR_LM(3) = 0x00;

     /* Channel 0 MCAN0RX P5.0 */
     FBL_PFCE(5)    |=   (1 << 0); /* Set bit 0 in PFCE5 for CAN mode */
     FBL_PFC(5)     &=  ~(1 << 0); /* Clear bit 0 to select CAN mode */
     FBL_PM(5)      |=   (1 << 0); /* Set Pin 0 as input */
     FBL_PMC(5)     |=   (1 << 0); /* Enable alternative port function */
     FBL_PU(5)      |=   (1 << 0); /* Enable pull-up */

     /* Channel 0 MCAN0TX P5.1*/
     FBL_PFCE(5)    |=   (1 << 1); /* Set bit 1 in PFCE5 for CAN mode */
     FBL_PFC(5)     &=  ~(1 << 1); /* Clear bit 1 to select CAN mode */
     FBL_PM(5)      &=  ~(1 << 1); /* Clear Pin 1 as output */
     FBL_PMC(5)     |=   (1 << 1); /* Enable alternative port function */
     FBL_PU(5)      |=   (1 << 1); /* Enable pull-up */
  
   /* Channel 0 STDBY P5.12*/
     FBL_PM(5) &= ~(1 << 12);
     FBL_P(5)  &= ~(1 << 12);
  /* GPIO for Test - P6.0 */
     FBL_P(6) &= 0xFE;   /* Turn off */
     FBL_PM(6)  &= 0xFE; /* Output mode */
     FBL_PMC(6) &= 0xFE; /* Port mode */
     /* Turn on */
     FBL_P(6) |= 0x01;
...
{
#if 0
   static Can_PduType progRequest;
   static uint8 myMsg[8];
#endif
#if 0   
   if(flashlight == 0)
   {
	   ...
	   uint8 data[8] ={0x55u, 0x55u, 0x55u, 0x55u, 0xAAu, 0xAAu, 0xAAu, 0xAAu};
   ...
   if (flashlight > 500)
   {
	   ...
	   myMsg[0] = 0x01;                                
	   myMsg[1] = 0xD2;                                
	   progRequest.id = 0x600;                         
	   progRequest.length = sizeof(myMsg);             /* Set message length to 8 */
	   progRequest.sdu = (uint8*)&myMsg[0];
	   progRequest.swPduHandle = 0;
         
       Can_Write(0u, &progRequest);   /* MailBoxConfigIdx 0,  Private CAN */
#endif    
      //FBL_P(6) ^= 0x01; /* Toggle P6.0 */
